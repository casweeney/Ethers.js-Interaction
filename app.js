...
const getSomethingAsync = async () => {
    try {
        ...
        const [tSupply, tSymbol, tOwner] = await Promise.all([tokenContract.totalSupply(), tokenContract.symbol(), tokenContract.owner()]);
        const formatted = ethers.utils.formatEther(tSupply);
        console.log(tSupply.toString(), tSymbol, tOwner, "Result from promise all", formatted);

        // console.log(decimalValue, "decimal value ", supply);
    } 
    catch {
